#!/usr/bin/env python3
"""
Create Demo GIFs for Recycling Sorting Environment
Shows both random agent and trained agent behavior
"""
...
def capture_screen(screen):
    """Capture pygame screen as numpy array"""
    w, h = screen.get_size()
    raw = pygame.image.tostring(screen, 'RGB')
    return np.frombuffer(raw, dtype=np.uint8).reshape((h, w, 3))

def create_demo_gif():
    """Create GIF showing random agent behavior"""
    ...
    if frames:
        ...
        imageio.mimsave('results/random_agent_demo.gif', frames, fps=20)
        print("GIF saved to results/random_agent_demo.gif")
    
    return len(frames)

def create_trained_agent_gif():
    """Create GIF showing trained agent behavior"""
    print("Creating trained agent GIF...")
    
    try:
        from stable_baselines3 import PPO
        
        # Load best model (PPO Default performed best)
        model_path = "models/pg/ppo/ppo_final"
        ...
        if frames:
            ...
            imageio.mimsave('results/trained_agent_demo.gif', frames, fps=20)
            print("Trained agent GIF saved to results/trained_agent_demo.gif")
        
        return len(frames)
        
    except Exception as e:
        print(f"Failed to create trained agent GIF: {e}")
        return 0

def main():
    """Main function to create demo GIFs"""
