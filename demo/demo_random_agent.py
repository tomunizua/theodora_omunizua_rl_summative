...
def run_random_agent_demo():
    ...
    print("Actions: 0=Paper, 1=Plastic, 2=Organic, 3=Metal, 4=Discard, 5=Scan")
    print("Rewards: High rewards for correct sorting, penalties for wrong actions, time penalty for scanning")
    print("Terminal conditions: Bin capacity reached, too many wrong sorts")
    print("Press 'q' to quit, 'r' to reset, 'space' to pause/resume")
    
    # Initialize environment and renderer with new parameters
        ...
        while current_episode < max_episodes:
            ...
            while not done:
                ...
                if paused:
                    time.sleep(0.1)
                    continue
                # Take random action (including scan with lower probability)
                if random.random() < 0.2:  # 20% chance to scan
                    action = 5  # Scan
                else:
